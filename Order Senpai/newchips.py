...
while u < 55:
    print(u)
    with open("diceorders.txt") as f:
       lines = f.read().splitlines()
    f.close()

    with open("diceorders.txt", 'r') as filehandle:
    ...
    #thang = str(ithang)
    strthng = str(u)
    url = "https://api.sellbrite.com/v1/orders?page=" + strthng + "&limit=100"
        ...
        while i < len(listthing):
            if not order_num in lines:
                ...
                if sku:
                    ...
                    #print(cat_name)
                    if (cat_name == "Casino Dice" or cat_name == "Dice") and ("Vial" not in name):
